...
| EP-04 Sidecar infra | Locally strong, packaging locally proven for macOS arm64 protocol + packaged host smoke; dual onefile/onedir build flag now supported | 95% | JSON-RPC, controller, model manifest/download manager, installer fixture manifest file, persisted manifest/mirror settings schema, subprocess smoke pass, Python 3.11 ML venv validated, PyInstaller darwin-arm64 binary smoke passed, packaged sister app launches the bundled sidecar from `process.resourcesPath`. The sidecar build now accepts `--mode={onefile,onedir}` and the host resolver checks both layouts so cold-start optimisation is a release-job flag flip rather than a code change | First PyInstaller entry failed on package-relative imports; root `build:sidecar` exposed a bare-`python` clean-env defect. Packaged Electron also did not set `NODE_ENV=production`, so sidecar resolution had to key off `app.isPackaged`. Manifest tests originally pinned only the in-memory object path; a real JSON file fixture now exercises the production parser. Env/settings precedence needed an explicit resolver so release jobs can override operator settings without mutating disk state. The refreshed Gate D smoke still proves packaged sidecar launch after stricter operator evidence capture. Full platform packaging and real model mirror remain release gates. The 2026-06-18 production-ready pass added the onedir build path and pinned the dual-layout resolver |
| EP-05 Song learning | Local packaged/operator production proof passing with caveats; cold-start build-flag deferred to Gate D | 99% | Deterministic path works; production stage contracts and progress are wired; Demucs/WhisperX packages install/import; public-domain opt-in fixture passes in source mode; staged release-owned model cache runs with `LYRICUE_MODEL_CACHE_ONLY=1`; packaged sidecar returns TimingMaps with clean JSON-RPC stdout and 25/26 confident words in the final variance sample; packaged release smoke is scripted and passing at 24/26 confident words; native warnings are captured and locally certified for the current in-memory path; operator bridge production Learn Song pass returns a valid TimingMap at 24/26 confident words; operator cancellation/fallback is proven locally; packaged sister smoke proves the host can launch the bundled sidecar for rehearsal segmentation; cold-start optimisation is a build-flag away (--mode=onedir) | The first 30-second fixture clipped the final phrase and falsely failed the quality gate. Demucs local-repo loading failed under PyTorch 2.8 safe-load defaults until LyriCue scoped trusted local artifact loading. Packaged ML required targeted PyInstaller rules for WhisperX, Pyannote, and torchcodec metadata. Operator production alignment needed a longer timeout than deterministic learning and a source-mode ML venv override. Active ML cancellation had to terminate the sidecar because the sidecar JSON-RPC loop cannot process `cancel_job` while inside Demucs/WhisperX work. Cold-start onefile latency is now a per-release decision (smaller installer vs faster launch) rather than a code-debt item |
...
| EP-07 Audio input/beat detection | Hot-swap contract pinned; telemetry now in operator state envelope | 90% | Synthetic and pure module tests pass; AudioInput hot-swap behaviour pinned for mid-session device changes (start→stop→start with new deviceId releases the old device + acquires the new one); beatConfidence + tempoRatio + vadState added to operator state broadcast for live telemetry UIs | Physical microphone/loopback QA remains a hardware gate. The 2026-06-18 production-ready pass closed the hot-swap behavioural gap and surfaced audio telemetry to the operator UI |
| EP-08 VAD/STT correction | Binding contract documented; no-op + constant transcribers shipped for pre-binding integration | 85% | VAD, phrase matcher, rolling STT window/backpressure scaffold, correction evaluator, host-neutral live-safe correction controller, STT disable no-op, durable position JSONL logging with decision-to-log entry mapping, live-STT model manifest requirements, accent-folding NFD normalisation, no-op transcriber for pre-binding integration testing, constant transcriber for deterministic phrase-matching tests, and the docs/ep08-stt-binding-contract.md spec a real binding must satisfy | Whisper.cpp native addon remains the main missing platform-specific dependency; the original epic package name no longer resolves on npm. The no-op transcriber means the full STT pipeline can be exercised end-to-end without the native binding. Binding selection (whisper-rs napi vs faster-whisper sidecar call-back) is the one remaining EP-08 decision |
...
| EP-10 Operator UI | Complete for M2; save-error surface closed | 100% | D13-D18 closed; D-T13 background-save error surface closed via OperatorSaveError tracker (lastSaveError in operator state envelope); controls usable in sister mode; shared SettingsTab is reachable from the sister operator window through validated get/save IPC; sidecar manifest controls have component-level persistence coverage; persisted shortcuts and selected audio device load at startup and save through live operator commands; Settings overlay now has live Electron screenshot evidence and a packaged-smoke parser signal; dead editArrangement/toggleRehearsal IPC stubs removed | Hydration and keyboard-focus bugs were integration defects not caught by pure tests. Mounting settings exposed a renderer-bundle defect where broad core imports pulled Node-only modules into Vite; the fix required narrow UI/core subpath imports. Settings saves initially persisted but did not prove live shortcut rebroadcast until the Electron smoke bridge was expanded. The first Settings screenshot exposed a viewport overflow defect, so SettingsTab now constrains nested form controls and wraps sections responsively. Device selection originally survived tick broadcasts but not app restarts until it was wired to SettingsStore. The 2026-06-18 production-ready pass closed the silent-save-failure observability gap and removed dead IPC scaffolding |
...
| EP-13 Library manager | Locally strong; integrity invariants pinned end-to-end | 88% | ZIP `.lcbundle`, integrity, import/export, signing contracts, opt-in bounded bundle downloads, end-to-end integrity verification across the export → atomic-write → disk → import cycle including post-disk-write tamper detection | Original JSON-only bundle shape had to be replaced by ZIP; stalled remote bundle downloads needed explicit abort semantics. The 2026-06-18 production-ready pass added six integrity tests pinning the disk-roundtrip + tamper-detection contract |
| EP-14 Library hosting | Locally strong, externally unverified | 79% | Worker, setup script, signing/trust, GitHub mirror logic, project publish audit logging, target validation, org/campus credential-header validation, credential metadata validation, safe R2 key-segment validation, and local Worker tests exist | Real Cloudflare R2/KV/Worker + GitHub mirror credentials are required for production proof; local QA found the Worker accepted identity headers without validating them against the credential, could 500 on malformed KV credential metadata, and accepted unsafe payload IDs for R2 object keys, all now fixed before external deployment |
| EP-15 Identity/publishing | Locally strong, external publish proof pending | 92% | Identity, publish credentials, safe-storage backend, operator credential configure/clear bridge, mounted publish credential dialog with dev and packaged smoke screenshot evidence, publish dialog/browser, song `.lcbundle` export from the sister host, project publish mode, project metadata hydration, and per-target credential gating exist | Secure storage wiring had a high-severity gap and was fixed locally; renderer show IDs initially risked drifting into catalog song IDs until the host split local `showId` from library `songId`; credential smoke now proves dummy secrets do not leak into config JSON and the nested credential UI is reachable from default Settings state; prompt-based credential entry has been replaced by an operator dialog, but real packaged safe-storage and Worker credentials still need external proof |
...
| EP-19 Multilingual lyrics | Translated-primary projection closed | 97% | Translation editor, rendering, language swap, sizing, operator mounting, stale-draft protection, main-process translation IPC narrowing, timing-map schema section-reference validation, Electron smoke stale-payload guard, and projectTimingMapToPrimaryLanguage(map, language) — a pure-function projection that promotes a parallel-lyrics track to primary while demoting the original learned-language to a parallel track for round-trip toggling | Translated-primary karaoke needed a per-language projection rather than per-language learning, because section-level translations are the only operator-tractable input. The 2026-06-18 production-ready pass shipped the projection with section-granularity sweep; word-level alignment across languages is a deliberate non-goal |
...
12. Refreshed the local macOS arm64 packaged smoke after EP-15 credential-dialog hardening; the packaged app captured `11-publish-credential-dialog-operator.png`, launched the bundled sidecar, and passed with repo-root evidence.
13. Fixed packaged smoke `--output-dir` relative path resolution so release-job evidence lands under the repository root instead of `apps/sister/docs`.
