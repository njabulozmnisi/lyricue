...
| EP-10 Operator UI | Complete for M2 | 98% | D13-D18 closed; controls usable in sister mode; shared SettingsTab is reachable from the sister operator window through validated get/save IPC; sidecar manifest controls have component-level persistence coverage; persisted shortcuts load at startup and rebroadcast live after settings saves; Settings overlay now has live Electron screenshot evidence and a packaged-smoke parser signal | Hydration and keyboard-focus bugs were integration defects not caught by pure tests. Mounting settings exposed a renderer-bundle defect where broad core imports pulled Node-only modules into Vite; the fix required narrow UI/core subpath imports. Settings saves initially persisted but did not prove live shortcut rebroadcast until the Electron smoke bridge was expanded. The first Settings screenshot exposed a viewport overflow defect, so SettingsTab now constrains nested form controls and wraps sections responsively |
