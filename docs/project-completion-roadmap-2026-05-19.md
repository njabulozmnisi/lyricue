...
- TypeScript/Vitest: 746 tests passing.
...
| EP-08 VAD/STT correction | Partial | 75% | VAD, phrase matcher, rolling STT window/backpressure scaffold, correction evaluator, host-neutral correction controller, STT disable no-op, position JSONL logging, and live-STT model manifest requirements exist; SyncEngine accepts correction events | Whisper.cpp native addon remains the main missing platform-specific dependency; the original epic package name no longer resolves on npm |
...
| EP-18 Arrangement builder | Mostly complete | 85% | Drag/drop, parser, named arrangements, operator persistence, refreshed modal hydration, and current-section save guards exist | Modal mounting and persistence were the real defects, not the pure arrangement logic; same-ID arrangement updates exposed stale modal state risk |
