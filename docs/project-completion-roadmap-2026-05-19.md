...
- TypeScript/Vitest: 758 tests passing.
...
| EP-19 Multilingual lyrics | Mostly complete | 92% | Translation editor, rendering, language swap, sizing, operator mounting, stale-draft protection, main-process translation IPC narrowing, and timing-map schema section-reference validation exist | Translated-primary karaoke needs a learned timing map per primary language; mounted editor QA surfaced stale timing-map draft, full-map IPC overwrite, and schema reference-drift hazards before they could become operator data loss |
