...
**Scope:** Production-mode Learn Song through the sister-mode operator bridge: operator progress IPC, cancellation IPC, main-process `learn_song` handler, source ML sidecar launch, Demucs + WhisperX alignment, TimingMap validation, manual fallback, and evidence capture.
**Environment:** Local macOS arm64 dev run; `LC_DEPLOYMENT_MODE=sister`, `LC_E2E_MODE=1`, `LC_CAPTURE_EVIDENCE=1`, `LC_CAPTURE_OPERATOR_TOOLS=1`, `LC_CAPTURE_PRODUCTION_LEARN_SONG=1`, `LC_CAPTURE_PRODUCTION_LEARN_SONG_CANCEL=1`, `LC_SIDECAR_PYTHON=python-sidecar/.venv-ml/bin/python`; staged model cache under `build/models/release`; offline/cache-only flags enabled.
**Status:** Pass-with-caveats

## Executive summary

Production-mode Learn Song now completes through the operator bridge and returns a valid TimingMap from the Amazing Grace fixture. It also cancels an active production alignment job by terminating the sidecar process, then falls back to the manual preview path.

No **CRITICAL** defects remain in this pass. Two **HIGH** defects and two **LOW** defects were fixed: timeout budget, source ML interpreter override, host-side cancellation, and manual fallback button behavior.
...
- Evidence paths: `docs/qa-reports/evidence/ep05-operator-production-learn-song-2026-06-06/production-learn-song-summary.json` and `docs/qa-reports/evidence/ep05-operator-production-learn-song-2026-06-06/production-learn-song-cancel-summary.json`.
...
| EP05-OP-07 | Production Learn Song cancellation | Live operator | Cancel interrupts active production alignment and rejects the in-flight request | Cancelled at `demucs`; sidecar terminated with `SIGTERM`; request rejected cleanly | Pass |
| EP05-OP-08 | Manual fallback after learning failure | Live operator | Failed/cancelled Learn Song can continue as manual preview without re-running sidecar | Component test reaches preview after learning rejection; `learnSong` called once | Pass |
...
Symptom: The sidecar exposed `cancel_job`, but an active production `learn_song` job could not actually be interrupted from the operator UI because the JSON-RPC server processes one request at a time.

Root cause: Cancellation was implemented as a normal sidecar request. While the sidecar was inside Demucs/WhisperX work, it could not read and process the queued cancel request.

Latency: Unit cancellation only covered a pre-cancelled job checkpoint; no operator path verified cancellation during active ML inference.

Repro steps: Start production Learn Song from the operator bridge and try to cancel while `demucs` or `whisperx` is active.

Evidence: `production-learn-song-cancel-summary.json` shows cancellation at `demucs`, `cancelled: true`, strategy `terminate-sidecar`, and error `Sidecar exited during request 'learn_song' (code=null, signal=SIGTERM)`.

Fix status: Fixed. `packages/core/src/sidecar/sidecar-controller.ts` now exposes `terminate()`, and `apps/sister/src/main.ts` wires `lyricue:operator:cancel-learn-song` with sender validation.

### D-EP05-OP-04 — **LOW**

Symptom: After Learn Song failed, the wizard displayed `Manual-mode fallback ready` but the primary button still said `Start learning`, so clicking it retried the sidecar instead of creating the manual preview.

Root cause: `LearnSongWizard` used `audioFileName` alone to choose the progress-step action label and did not treat `learnError` as a manual-fallback state.

Latency: Existing tests covered success and manual skip paths but not the rejection/fallback path.

Repro steps: Inject a rejecting `learnSong` callback on the progress step and click `Start learning`.

Evidence: `packages/ui/src/LearnSongWizard.test.ts` now verifies the rejection message, manual-fallback label, `Create manual preview` button, preview transition, and single `learnSong` invocation.

Fix status: Fixed. `packages/ui/src/LearnSongWizard.svelte` now treats a prior learning error as manual fallback and advances to preview without re-invoking the sidecar.
...
- Operator cancellation IPC is sender-validated and interrupts active ML work by terminating the sidecar process; the next request can respawn the sidecar through normal `ensureRunning()`.
...
| EP-05 operator production Learn Song | 0 | 2 | 0 | 2 | 0 | 4 fixed |

## Recommendations before production shipping

1. **HIGH** Add a release smoke that launches the packaged sister app and runs `LC_CAPTURE_PRODUCTION_LEARN_SONG=1` and `LC_CAPTURE_PRODUCTION_LEARN_SONG_CANCEL=1` against the packaged sidecar, not only source `.venv-ml`.
2. **MEDIUM** Resolve or certify the torchcodec/FFmpeg and torchaudio native dependency warnings in the signed/notarized release artifact.
3. **LOW** Keep the operator production evidence harness explicit-only; it should not run during normal fast smoke because it is intentionally heavyweight.

## Final verdict

EP-05 production Learn Song is locally pass-with-caveats through the sister-mode operator bridge. The operator can launch production alignment, receive progress, cancel active ML work, fall back to manual preview, and get a validated TimingMap with acceptable confidence from the public-domain fixture. The remaining gap is packaged-app release certification for the same operator evidence paths plus native dependency warning resolution.
