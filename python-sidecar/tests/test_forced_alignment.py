...
from lyricue_sidecar.forced_alignment import _forced_segments, align_vocals
...
def test_align_vocals_wraps_runner_failure():
    with pytest.raises(JsonRpcError) as exc:
        align_vocals(vocals(), sections(), runner=lambda *_args: (_ for _ in ()).throw(RuntimeError("aligner exploded")))
